...
async function replaceWithUnsplashImages(articleContent, topic) {
  const matches = [...articleContent.matchAll(/<img[^>]+src=["']https:\/\/picsum\.photos\/[^"']+["'][^>]*>/g)];
  ...
  for (let i = 0; i < matches.length; i++) {
    try {
      ...
      const unsplashImage = res.data?.urls?.regular;
      const altText = res.data?.alt_description || topic;

      articleContent = articleContent.replace(matches[i][0], 
        `<img src="${unsplashImage}" alt="${altText}" style="border-radius: 16px; margin: 2em 0;" />`
      );
    } catch (err) {
